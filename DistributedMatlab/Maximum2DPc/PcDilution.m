...
%               (default = [0.5 2.0]).
%            - params.dLfLevel - Log10 cov. scale factor spacing, initial
...
%    PcMax   - Maximum Pc value found for any value of applied cov. scale
%              factor, if PcOne is found to be in dilution region.
%    SfMax   - Covariance scale factor for which maximum Pc occurs.
%    Pc      - The Pc buffer created in the iterative search for PcMax.
%    Sf      - Covariance scale factor buffer created in search for PcMax.
%              (Plotting y=Pc vs x=Sf creates a Pc dilution curve.)
...
% ----------------- BEGIN CODE -----------------
    ...
    % Initial covariance scale factors and log10 values
    
    if ~isfield(params,'SfInit') || isempty(params.SfInit)
        params.SfInit = [0.5 2.0];
    end
    
    % Log10 cov. scale factor spacing, initial and refinement levels
    ...
    % Find the Pc value for complete cov. down-scaling (i.e., Sf = 0)
    ...
    % Log10 cov. scale factor spacing
    ...
    if isnan(PcMax)
        ...
        SfMax = Sf(imax);
        Diluted = SfMax < 1; % Dilution occurs only for cov. scaling < 1
        if Diluted
...
%                                of searched scale factors.
% D.Hall         | 2021-OCT-14 | Added comments to make it explicitly
%                                clear that the returned scale factors
%                                multiply the covariance matrix, so the
%                                square-root of these scale factors should
%                                be used for pre/post covariance
%                                multiplication.
% L. Baars       | 2023-FEB-22 | Added pathing information for the SDK
%                                reorganization.
