...
% ----------------- BEGIN CODE -----------------
    ...
    % Leap seconds data
    persistent LeapSecInfo;
    
    if (isempty(LeapSecInfo))
        % Get leap second info from a .mat file (LeapSeconds.mat)
        [p,~,~] = fileparts(mfilename('fullpath'));
        leapSecFile = fullfile(p, '../../../DataFiles/TimeCons/LeapSeconds.mat');
        load(leapSecFile,'LeapSecInfo');
    end
...
%                                functionality.
% L. Baars       | 10-18-2022 |  Updated to account for new LeapSecInfo
%                                file location.