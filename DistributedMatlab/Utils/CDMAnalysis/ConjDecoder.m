...
%
%   OCMDB - [nx218], [nx234], or [nx235] matrix generated by CARA's OCMDB
%           extraction process.
%
...
% ----------------- BEGIN CODE -----------------
    ...
    elseif narg == 2
        ...
        if ischar(varg{1}) && isa(varg{2},'double') && isequal(size(varg{2}),[1 1])
            ...
            readCdm = true;
            HBR = varg{2};
        elseif isa(varg{1},'double') && (size(varg{1},2) == 218 || size(varg{1},2) == 234 || size(varg{1},2) == 235) && ...
                ischar(varg{2})
            OCMDB = varg{1};
            conjID = varg{2};
            parseConjID = true;
    ...
    elseif narg == 3
        ...
        if isa(varg{1},'double') && (size(varg{1},2) == 218 || size(varg{1},2) == 234 || size(varg{1},2) == 235) && ...
                istable(varg{2}) && ...
                ischar(varg{3})
            OCMDB = varg{1};
            DBMetadata = varg{2};
    ...
    if readCdm
        ...
        % m/s)
        if size(DB,2) == 234 || size(DB,2) == 235
...
% L. Baars       | 2025-OCT-10 | Added support for OCMDB inputs with 235
%                                columns.
