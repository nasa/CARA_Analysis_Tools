...
%         hbar = parfor_progressbar(N,'','Name','Parfor Progress');  %create the progress bar
%         parfor i=1:N,
%         pause(rand*2);       % computation
...
properties (SetAccess=protected, GetAccess=protected, Hidden)
    ...
    computationTimer; % Time of computation used to estimate additional time required
end
...
methods
    ...
    % Create a new progress bar with N_init iterations before completion.
        ...
        % Create Timer to estimate additional computation time
        this.computationTimer = tic;
        pause(1)
        
        % Create timer to periodically update the waitbar in the GUI thread.
        this.htimer = timer( 'ExecutionMode','fixedSpacing', 'Period',1, ...
...
methods (Access=protected, Hidden)
    ...
    % Check the IPC file and update the waitbar with progress.
        ...
        if ishandle(this.wbh)
            g = toc(this.computationTimer)*(1-this.percent)/this.percent;
            % Make String for Updated waitbar message
            if g <= 120 || g == inf% Write Projected ETA in Seconds if ETA < 2 minutes
                messageString = {['Progress: ' num2str(round(this.percent*this.N),'%d') ' of ' num2str(this.N,'%d')];...
                                ['Projected ETA: ' num2str(round(g),'%d') ' Seconds']};
            elseif g > 120 && g <= 60*60*2 % Write Projected ETA in Minutes if ETA < 2 Hours
                messageString = {['Progress: ' num2str(round(this.percent*this.N),'%d') ' of ' num2str(this.N,'%d')];...
                                ['Projected ETA: ' num2str(g/60,'%0.2f') ' Minutes']};
            elseif g > 60*60*2 && g <= 60*60*24 % Write Projected ETA in Hours if ETA < 1 Day
                messageString = {['Progress: ' num2str(round(this.percent*this.N),'%d') ' of ' num2str(this.N,'%d')];...
                                ['Projected ETA: ' num2str(g/60/60,'%0.2f') ' Hours']} ;
            else % Write Projected ETA in Days
                messageString = {['Progress: ' num2str(round(this.percent*this.N),'%d') ' of ' num2str(this.N,'%d')];...
                                ['Projected ETA: ' num2str(g/60/60/24,'%0.2f') ' Days']};
            end
             
            waitbar( this.percent, this.wbh ,messageString);
        else
