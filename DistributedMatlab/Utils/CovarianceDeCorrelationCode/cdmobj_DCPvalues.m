...
%
% Syntax: [DCPSig,DCPSens] = cdmobj_DCPvalues(X,cdmobj)
%
% Inputs:
%
...
% ----------------- BEGIN CODE -----------------
    ...
    if isfield(cdmobj,'DCP_SENSITIVITY_RTN_POS')
        if isempty(cdmobj.DCP_SENSITIVITY_RTN_POS)
            GPOS = [];
        else
            c = strsplit(cdmobj.DCP_SENSITIVITY_RTN_POS);
            if numel(c) ~= 3
                warning('DCP_SENSITIVITY_RTN_POS has wrong number of parameters');
                GPOS = [];
            else
    ...
    if isfield(cdmobj,'DCP_SENSITIVITY_RTN_VEL')
        if isempty(cdmobj.DCP_SENSITIVITY_RTN_VEL)
            GVEL = [];
        else
            c = strsplit(cdmobj.DCP_SENSITIVITY_RTN_VEL);
            if numel(c) ~= 3
                warning('DCP_SENSITIVITY_RTN_VEL has wrong number of parameters');
                GVEL = [];
            else
    ...
    elseif any(emptySPV)
        ...
        warning('Incomplete set of DCP values found in CDM object data');
        DCPSens = [];
        return;
    end
...
% D.Hall         | 2020-APR-02 | Added modifications allow DCP fields to be
%                                empty, interpreted the same as if they
%                                don't exist at all.
%