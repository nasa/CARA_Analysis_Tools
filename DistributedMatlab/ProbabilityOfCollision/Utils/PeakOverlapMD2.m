function [MD2,Xu,Ps,Asdet,Asinv,POPconverged,aux] = PeakOverlapMD2(t,t10,Eb10,Qb10,t20,Eb20,Qb20,HBR,EMD2,params)
...
if POPconverged > 0
    ...
    % % Testing for one Obj 48901 DCP case
    % warning('Setting Lclip to zero for testing (e.g., Obj 48901)'):
    % Lclip = 0;
    [Veig,Leig] = eig(As); Leig = diag(Leig); Leig(Leig < Lclip) = Lclip;
    Asdet = Leig(1) * Leig(2) * Leig(3);
    Asinv = Veig * diag(1./Leig) * Veig';
    aux.As = As; aux.AsLeig = Leig; aux.AsVeig = Veig; aux.AsLclip = Lclip;
