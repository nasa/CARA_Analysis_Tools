...
% This function is a vectorized version of an eigenvalue and eigenvector
% solver for 2x2 symmetric matrices.
% 
% Input:
%   Araw - (nx3) matrix representing n symmetric matrices
%           Araw(:,1) is the 1,1 component for each symmetric matrix
%           Araw(:,2) is the 1,2 and 2,1 component for each symmetric
%           matrix
%           Araw(:,3) is the 2,2 component for each symmetric matrix
% Outputs:
%   v1 - (nx2) matrix representing n of the 1st eigenvectors
%   v2 - (nx2) matrix representing n of the 2nd eigenvectors
%   l1 - (nx1) matrix representing n of the 1st eigenvalues
%   l2 - (nx2) matrix representing n of the 2nd eigenvalues
function [v1, v2, l1, l2] = eig2x2(Araw)
    ...
    zeroBreg = b == 0 & a <= c;
    if sum(zeroBreg) > 0
        v1(zeroBreg,:) = repmat([1 0],sum(zeroBreg),1);
        v2(zeroBreg,:) = repmat([0 1],sum(zeroBreg),1);
    end
    zeroBrev = b == 0 & a > c;
    if sum(zeroBrev) > 0
        v1(zeroBrev,:) = repmat([0 1],sum(zeroBrev),1);
        v2(zeroBrev,:) = repmat([1 0],sum(zeroBrev),1);
    end
end
